...
expName = 'routeOneExP'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ReGame-XR Lab\\ExerBike_CBH\\psychopyExp\\RouteOne_EXP\\routeOneExP_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    RouteOne = visual.MovieStim(
        win, name='RouteOne',
        filename='C:/Users/ReGame-XR Lab/Desktop/Videos/Video1Corrected.mp4', movieLib='ffpyplayer',
    ...
    trailComponents = [RouteOne, SpanishRepeat01_noPedal, bikebuddy_SlowRepeat01, bikebuddy_FastRepeat01, key_resp]
    ...
    while continueRoutine:
        ...
        # if RouteOne is starting this frame...
        if RouteOne.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            RouteOne.frameNStart = frameN  # exact frame index
            RouteOne.tStart = t  # local t and not account for scr refresh
            RouteOne.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(RouteOne, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'RouteOne.started')
            # update status
            RouteOne.status = STARTED
            RouteOne.setAutoDraw(True)
            RouteOne.play()
        
        # if RouteOne is stopping this frame...
        if RouteOne.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > RouteOne.tStartRefresh + 2007-frameTolerance:
                # keep track of stop time/frame for later
                RouteOne.tStop = t  # not accounting for scr refresh
                RouteOne.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'RouteOne.stopped')
                # update status
                RouteOne.status = FINISHED
                RouteOne.setAutoDraw(False)
                RouteOne.stop()
    ...
    RouteOne.stop()  # ensure movie has stopped at end of Routine
    # check responses
