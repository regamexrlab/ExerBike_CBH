...
expName = 'routeTwoExP'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ReGame-XR Lab\\ExerBike_CBH\\psychopyExp\\RouteTwo_EXP\\routeTwoExP_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    RouteTwo = visual.MovieStim(
        win, name='RouteTwo',
        filename='C:/Users/ReGame-XR Lab/Desktop/Videos/Video2Corrected.mp4', movieLib='ffpyplayer',
    ...
    trailComponents = [RouteTwo, SpanishRepeat01_noPedal, bikebuddy_SlowRepeat01, bikebuddy_FastRepeat01, key_resp]
    ...
    while continueRoutine:
        ...
        # if RouteTwo is starting this frame...
        if RouteTwo.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            RouteTwo.frameNStart = frameN  # exact frame index
            RouteTwo.tStart = t  # local t and not account for scr refresh
            RouteTwo.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(RouteTwo, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'RouteTwo.started')
            # update status
            RouteTwo.status = STARTED
            RouteTwo.setAutoDraw(True)
            RouteTwo.play()
        
        # if RouteTwo is stopping this frame...
        if RouteTwo.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > RouteTwo.tStartRefresh + 1800-frameTolerance:
                # keep track of stop time/frame for later
                RouteTwo.tStop = t  # not accounting for scr refresh
                RouteTwo.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'RouteTwo.stopped')
                # update status
                RouteTwo.status = FINISHED
                RouteTwo.setAutoDraw(False)
                RouteTwo.stop()
    ...
    RouteTwo.stop()  # ensure movie has stopped at end of Routine
    # check responses
