...
expName = 'routeThreeExP'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ReGame-XR Lab\\ExerBike_CBH\\psychopyExp\\RouteThree_EXP\\routeThreeExP_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    RouteThree = visual.MovieStim(
        win, name='RouteThree',
        filename='C:/Users/ReGame-XR Lab/Desktop/Videos/Video3CorrectedRoute3_0911.mp4', movieLib='ffpyplayer',
    ...
    trailComponents = [RouteThree, SpanishRepeat01_noPedal, bikebuddy_SlowRepeat01, bikebuddy_FastRepeat01, key_resp]
    ...
    while continueRoutine:
        ...
        # if RouteThree is starting this frame...
        if RouteThree.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            RouteThree.frameNStart = frameN  # exact frame index
            RouteThree.tStart = t  # local t and not account for scr refresh
            RouteThree.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(RouteThree, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'RouteThree.started')
            # update status
            RouteThree.status = STARTED
            RouteThree.setAutoDraw(True)
            RouteThree.play()
        
        # if RouteThree is stopping this frame...
        if RouteThree.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > RouteThree.tStartRefresh + 1727-frameTolerance:
                # keep track of stop time/frame for later
                RouteThree.tStop = t  # not accounting for scr refresh
                RouteThree.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'RouteThree.stopped')
                # update status
                RouteThree.status = FINISHED
                RouteThree.setAutoDraw(False)
                RouteThree.stop()
    ...
    RouteThree.stop()  # ensure movie has stopped at end of Routine
    # check responses
